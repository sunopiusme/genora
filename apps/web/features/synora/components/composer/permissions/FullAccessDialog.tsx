...
import styles from "./FullAccessDialog.module.css";

/* ─────────────────────────────────────────
   Модальное подтверждение «Полного доступа».

   Показывается один раз за сессию при первом
   выборе уровня full. Рендерится через портал
   поверх всех экранов: на десктопе — панель
   до 700px по центру, на мобильных — нижняя
   шторка на всю ширину.
   ───────────────────────────────────────── */
...
export function FullAccessDialog({ onAccept, onDecline }: Props) {
  ...
  /* Фокус на основную кнопку + Escape = отказ + блокируем
     прокрутку страницы, пока модалка открыта. */
  useEffect(() => {
...
/* Solar shield-warning-linear — та же иконка, что у пункта
   «Полный доступ» в PermissionPicker. */
