...
1. `AI_GATEWAY_API_KEY` — Vercel AI Gateway. Модель передаётся строкой каталога (`anthropic/claude-sonnet-4.5`), AI SDK направляет запрос в шлюз автоматически. Используется в предпросмотре v0 и на Vercel.
2. `ANTHROPIC_GATEWAY_BASE_URL` + `ANTHROPIC_GATEWAY_API_KEY` — Anthropic-совместимый шлюз (локально: `http://127.0.0.1:8000/v1`). Значение без суффикса `/v1` (`http://127.0.0.1:8000`) тоже принимается. Используется в локальной разработке, где `AI_GATEWAY_API_KEY` не задан.
...
2. Добавить соответствие `modelId → id модели шлюза` в `VERCEL_GATEWAY_MODELS` и `ANTHROPIC_GATEWAY_MODELS` в `api/anthropic-chat.ts`.
